<#	
	...
	-------------------------------------------------------------------------
	 ...
	 Modifications (8/18/2015 2:04 PM):
	 (Ken Efta) Refined the Invoke-ReadFromTwitterStream function to 
	 retrieve a subset of fields returned as part of the Twitter Stream
	 API: CreatedDate, TweetBody, Langauge, FavCount, Retweeted, UserHandle, URL
	 
	 Other fields can easily be added. 
	 
	 TO DO: 
		* write column headers into the CSV file generated by the Invoke-ReadFromTwitterStream
		* optimize code; have someone review code
           ...
           Param(
                ...
                while($true)
                {
                  ...
                  try
                  {
                    ...
                    #Trying to figure out how best to auto-generate a header row in the CSV OutFile that represents the line-by-line tweet data we're capturing; for now the fields are represented in the $headerRow comment
                    #$headerRow = "CreatedDate","TweetBody","Langauge","FavCount","Retweeted","UserHandle","URL"
                    #$headerRow -join "," >> $OutFilePath
                    ...
                    while ($true) 
                    {
                            ...
                            $rawTweetData = $Line | ConvertFrom-Json
                            $resultsarray =@()

                            foreach ($item in $rawTweetData) {
                            ...
                            $CSVresultsArray = $resultsarray | ConvertTo-Csv

                            #Strip off Type declaration and header, using only the Tweet row info
                            $null, $null, $tweetRowOnly = $CSVresultsArray
                            
                            Add-Content $OutFilePath $tweetRowOnly
                            Write-Host $tweetRowOnly
